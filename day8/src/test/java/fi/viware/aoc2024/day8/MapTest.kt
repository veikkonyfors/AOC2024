...
class MapTest(){
    ...
    fun puzz1Tests(){
        ...
        antennaLocations = map.calculateAntinodeLocationsForaFrequency(map.frequencies.first())
        s = antennaLocations.joinToString { "(" + it.first.toString() + ", " + it.second.toString() + ")"}
        //println("$s")
        ...
        day.antennaMap.frequencies.forEach {
            totalAntinodeList += day.antennaMap.calculateAntinodeLocationsForaFrequency(it)
        }
